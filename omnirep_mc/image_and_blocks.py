...
# representation: start positions for same-color blocks; encoding: block size and color of each block
...
NUM_BLOCKS = 5000 # how many blocks compose the picture
...
IMAGE_FOLDER = 'images/adamhand/1/'
IMAGE_FILE = 'adamhand' 
...
def rep_init_population(data_dict): # initialize population of representations, individual = list of start positions (pixel indexes)
    width, height = data_dict["width"], data_dict["height"]
    return [ [ randint(0, width*height-1) for j in range(REP_GENOME_SIZE)] for i in range(REP_POP_SIZE) ]

def enc_init_population(ENC_EVOLVE, data_dict): # initialize population of encodings, individual = list of [block size, color]
    if ENC_EVOLVE:
        return [ [ [randint(MIN_BLOCK_SIZE,MAX_BLOCK_SIZE), randint(0,NUM_COLORS-1)] for j in range(ENC_GENOME_SIZE)] for i in range(ENC_POP_SIZE)] 
...
def draw_image(enc_ind, rep_ind, data_dict):  
    ...
    for i in range(REP_GENOME_SIZE):
        start = rep_ind[i]
        end = start + enc_ind[i][0]
        if end > width*height: end = width*height
        for j in range(start,end):
            pixels[j] = enc_ind[i][1]
...
def rep_mutation(ind, data_dict): # mutation of an individual in the representation population
    if (random() >= REP_PROB_MUTATION):
        ...
        ind[gene] = randint(0, width*height-1)             
        return(ind)  
...
def enc_mutation(ind): # mutation of an individual in the encoding population
    if (random() >= ENC_PROB_MUTATION):
        ...
        ind[gene] = [randint(MIN_BLOCK_SIZE,MAX_BLOCK_SIZE), randint(0,NUM_COLORS-1)]
        return(ind)     
